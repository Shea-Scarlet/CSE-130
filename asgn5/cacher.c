...
char *read_line();
bool contains(CacheItem *item, char *data);
...
int main(int argc, char *argv[]) {
    ...
    while ((line = read_line()) != NULL) {
        ...
        if (strcmp(policy, "-L") == 0) {
            result = insert_lru(cache, line);
        } else if (strcmp(policy, "-C") == 0) {
            result = insert_clock(cache, line);
        } else {
            result = insert_fifo(cache, line);
        }
        if (result == 0) {
            printf("HIT\n");
        } else {
